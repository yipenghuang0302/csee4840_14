...
The following algorithm describes the Jacobian Transpose method of solving the inverse kinematics problem. As input we are given a set of D-H parameters that fully define
...
\While{the $l^2$ norm of $e$ is greater than $\epsilon$}{
	...
	Let $u = JJ^T e$\;
	Let $\lambda = \frac{u \cdot e}{u \cdot u}$\; 
	Let $\Delta \theta = \lambda J^T e$ be a vector whose $i$th component is a change in joint $i$'s angle. Note that
